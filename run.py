...
while (1):
    ...
    if frame_count%2 == 0:
        continue
    if int((time.time()-start_time)) % 180 == 0:
        traffic_speed_left = 0
        traffic_speed_right = 0
        traffic_flow_left = 0
        traffic_flow_right = 0
    ...
    for blob in tracked_blobs:
        ...
        if blob['trail'][0][1] < REF_Y-REGION_DEPTH and blob['trail'][0][1] > REF_Y-REGION_DEPTH-5:
            if not blob['counted']:
                blob['counted'] = True
                if blob['trail'][0][0] < DIV_X:
                    cv.line(frame, (40,REF_Y-REGION_DEPTH), (DIV_X, REF_Y-REGION_DEPTH), (255,0,0), 2)
                    if blob['label'] == 'Light':
                    ...
                    cv.line(frame, (DIV_X,REF_Y-REGION_DEPTH), (REF_X - 40, REF_Y-REGION_DEPTH), (255,0,0), 2)
                    if blob['label'] == 'Light':
        ...
        if blob['trail'][0][1] < REF_Y and blob['trail'][0][1] > REF_Y-5:
            if not blob['counted']:
                ...
                if blob['trail'][0][0] < DIV_X:
                    ...
                    cv.line(frame, (DIV_X,REF_Y), (REF_X, REF_Y), (255,0,0), 2)
                    if blob['label'] == 'Light':
                        vehicle_count_right_L += 1
                    elif blob['label'] == 'Medium':
                        vehicle_count_right_M += 1
    ...
    traffic_flow_left = (vehicle_count_left_L+vehicle_count_left_M+vehicle_count_left_H-temp_count_left)*3600/((time.time()-start_time) % 180)
    traffic_flow_right = (vehicle_count_right_L+vehicle_count_right_M+vehicle_count_right_H-temp_count_right)*3600/((time.time()-start_time) % 180)
    traffic_flow_left = round(traffic_flow_left, 2)
    traffic_flow_right = round(traffic_flow_right, 2)
    ...
    cv.putText(frame, str(traffic_flow_left), (120,70), font, 0.8, (255,255,0), 2)
    cv.putText(frame, str(traffic_flow_right),(DIV_X+80+90,70), font, 0.8, (255,255,0), 2)
